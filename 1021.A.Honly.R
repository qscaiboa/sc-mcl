...
saveRDS (Apt, file= "./Data/Object.PatientA&PBMC.human_only.RDS")

 png("ZZZ.1020_Apt.human_only.tsne.png", width = 600, height = 600)
 DimPlot(Apt, reduction = "tsne", label = TRUE)
 dev.off()

 png("ZZZ.1020_Apt.human_only.tsne.Finalname.png", width = 900, height = 600)
 DimPlot(Apt, reduction = "tsne", label = TRUE, group.by= "Finalname")
 dev.off()

 png("ZZZ.1020.Apt.human_only.tsne.byspecies.png", width = 600, height = 600)
 DimPlot(Apt, reduction = "tsne", label = TRUE, group.by= "species")
 dev.off()
 
 png("ZZZ.1020.Apt.human_only.tsne.byPt.png", width = 600, height = 600)
 DimPlot(Apt, reduction = "tsne", label = TRUE, group.by= "Pt")
 dev.off()

 png("ZZZ.1020.Apt.human_only.tsne.splitbyFinalname.groupbyPt.png", width = 1200, height = 800)
 DimPlot(Apt, reduction = "tsne", label = TRUE, split.by= "Finalname",group.by= "species", ncol=8)
 dev.off()
 
 png("ZZZ.1020.Apt.human_only.tsne.splitbyFinalname.png", width = 1200, height = 800)
 DimPlot(Apt, reduction = "tsne", label = TRUE, split.by= "Finalname", ncol=8)
 dev.off()
  
 png("ZZZ.1020_Apt.human_only.tsne.FeaturePlot.PBMC.png", width = 1200, height = 800)
...
library(harmony, lib.loc = "/rsrch3/scratch/lym_myl_rsch/qcai1/R/library/4.0.0")
 ...
 Apt<- FindClusters( Apt, resolution = 0.5)
saveRDS (Apt, file= "./Data/Object.PatientA&PBMC.harmony.human_only.RDS")

 png("ZZZ.1020_Apt.human_only.harmony.umap.png", width = 600, height = 600)
 DimPlot(Apt, reduction = "umap", label = TRUE)
 dev.off()

 png("ZZZ.1020_Apt.human_only.harmony.umap.Finalname.png", width = 900, height = 600)
 DimPlot(Apt, reduction = "umap", label = TRUE, group.by= "Finalname")
 dev.off()

 png("ZZZ.1020.Apt.human_only.harmony.umap.splitbyFinalname.groupbyPt.png", width = 1200, height = 800)
 DimPlot(Apt, reduction = "umap", label = TRUE, split.by= "Finalname",group.by= "species", ncol=8)
 dev.off()
 
 png("ZZZ.1020.Apt.human_only.harmony.umap.splitbyFinalname.png", width = 1200, height = 800)
 DimPlot(Apt, reduction = "umap", label = TRUE, split.by= "Finalname", ncol=8)
 dev.off()
 
  
 png("ZZZ.1020.Apt.human_only.harmony.umap.byPt.png", width = 600, height = 600)
 DimPlot(Apt, reduction = "umap", label = TRUE, group.by= "Pt")
 dev.off()
 
 
 png("ZZZ.1020_Apt.human_only.harmony.umap.FeaturePlot.PBMC.png", width = 1200, height = 800)
 FeaturePlot(Apt, features = c("IL7R", "CCR7",
                               "S100A4",
                               "CD14", "LYZ",
							   "MS4A1","CD79A", "CD79B",
