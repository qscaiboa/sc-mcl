...
readPDX <- function(samplename, hmDIR, humanDIR, mouseDIR, figureDIR, dataDir) {
  ...
  ##########################
  fileDIRh = paste(workDIRh, initialSampleName, "/outs/filtered_feature_bc_matrix", sep="")
  ...
  fileDIRm = paste(workDIRm, initialSampleName, "/outs/filtered_feature_bc_matrix", sep="")
  Data_tmp_m<-Read10X(data.dir = fileDIRh )
  rownames(Data_tmp_m) <- toupper(rownames(Data_tmp_m))
  ObjectM<- CreateSeuratObject(counts = Data_tmp_m, project = "huamn", min.cells = 3, min.features=200)
